...
class GemConverter:
    ...
    def _convert_active_skill(self, active_skill):
        stat_conversions = {}
        for in_stat, out_stat in zip(
            active_skill["Input_StatKeys"], active_skill["Output_StatKeys"]
        ):
        ...
        r = {
            ...
            "weapon_restrictions": [
                ic["Id"] for ic in active_skill["WeaponRestriction_ItemClassesKeys"]
            ],
    ...
    def _convert_support_gem_specific(cls, granted_effect):
        return {
            ...
            "added_types": cls._select_active_skill_types(
        ...
        if multipliers is not None:
            ...
            for stat_type, multi in multipliers.items():
                if multi == 0 or multi == 33 or multi == 34 or multi == 50:
                    ...
                    print(
                        "Unknown multiplier (50) for " + gepl["GrantedEffectsKey"]["Id"]
                    )
                    req = 0
                else:
    ...
    def _convert_base_item_specific(self, base_item_type, obj):
        ...
        if key:
            obj["projectile_speed"] = self.relational_reader["Projectiles.dat"].index[
                "Id"
    ...
    def convert(
        ...
        tp_per_level = {
            level: self._to_tooltip(obj, level, gepls_dict.values(), for_level)
            for level, for_level in gepls_dict.items()
        }
        tooltip = {"per_level": tp_per_level}
        if len(gepls) > 0:
        ...
        if len(gepls) >= 1:
            ...
            if static is not None:
                ...
                for stats in (
                    pl["stats"] for pl in tp_per_level.values() if "stats" in pl
                ):
                    for i in range(len(stats)):
                        if (
                            stats[i] is not None
                            and "values" in stats[i]
                            and stats[i]["values"] is None
                        ):
                            del stats[i]["values"]
    ...
    def _to_tooltip(self, gem, level, all_levels, for_level):
        ...
        if has_base_item:
            ...
            if max_level is None or level >= max_level:
                p += " (Max)"
        properties.append({"text": p, "value": level})
        if "mana_multiplier" in for_level and any(
            l["mana_multiplier"] != 100 for l in all_levels
        ):
            properties.append(
                {"text": "Mana Multiplier: {0}%", "value": for_level["mana_multiplier"]}
            )
        if "mana_cost" in for_level and for_level["mana_cost"] > 0:
            ...
            if has_active_skill:
                ...
                if (
                    ActiveSkillType.mana_cost_is_reservation.name in types
                    and ActiveSkillType.totem.name not in types
                ):
                    p = "Mana Reserved: {0}"
                    if ActiveSkillType.mana_cost_is_percentage.name in types:
                        p += "%"
            properties.append({"text": p, "value": for_level["mana_cost"]})
        ...
        if "vaal" in for_level:
            properties.append(
                {"text": "Souls Per Use: {0}", "value": for_level["vaal"]["souls"]}
            )
            properties.append(
                {"text": "Can Store {0} Use", "value": for_level["vaal"]["stored_uses"]}
            )
        if "cooldown" in for_level:
            ...
            properties.append({"text": p, "values": vs})
        if "cast_time" in gem:
            properties.append(
                {"text": "Cast Time: {0} sec", "value": float(gem["cast_time"] / 1000)}
            )
        ...
        if any("damage_effectiveness" in l for l in all_levels):
            damage_effectiveness = (
                for_level["damage_effectiveness"]
        ...
        stats.extend(
            self._tooltip_stats(tf, for_level["stats"], are_quality_stats=False)
        )

        quality_stats = list(
            self._tooltip_stats(tf, for_level["quality_stats"], are_quality_stats=True)
        )
